...
class basemodel extends CI_Model {
    ...
    public function isProgramaFavorito($idConteudo) {
        ...
        $query = "SELECT COUNT(*) AS TOTAL FROM GP_DESKTOP WHERE ID_USUARIO = '$idUsuarioLogado' AND ID_CONTEUDO = '$idConteudo'";
    ...
    public function alterarFavoritacaoPrograma($nomeAba, $nomePrograma, $idConteudo, $controllerChamado, $enderecoIcone) {
        ...
        $query = "SELECT COUNT(*) AS TOTAL FROM GP_DESKTOP WHERE ID_USUARIO = '$idUsuarioLogado' AND NOME_ABA = '$nomeAba' AND NOME_PROGRAMA = '$nomePrograma' AND ID_CONTEUDO = '$idConteudo' AND CONTROLLER_CHAMADO = '$controllerChamado'";
        $consulta = $this->conBanco->query($query);
        ...
        if ($rs[0]->TOTAL == 0) {
            ...
            $query = "SELECT MAX(ID) AS MAXIMO FROM GP_DESKTOP";
            $consulta = $this->conBanco->query($query);
            $rs = $consulta->result();

            $novoId = $rs[0]->MAXIMO + 1;

            $query = "INSERT INTO GP_DESKTOP (ID, ID_USUARIO, NOME_ABA, NOME_PROGRAMA, ID_CONTEUDO, CONTROLLER_CHAMADO, ENDERECO_ICONE) VALUES('$novoId','$idUsuarioLogado','$nomeAba','$nomePrograma','$idConteudo','$controllerChamado','$enderecoIcone')";
            $this->conBanco->query($query);
            // retorna que inseriu
            return true;
        } else {
            // deleta
            $query = "DELETE FROM GP_DESKTOP WHERE ID_USUARIO = '$idUsuarioLogado' AND NOME_ABA = '$nomeAba' AND NOME_PROGRAMA = '$nomePrograma' AND ID_CONTEUDO = '$idConteudo' AND CONTROLLER_CHAMADO = '$controllerChamado'";
            $this->conBanco->query($query);
            //retorna que apagou
            return false;
        }
    ...
    public function getIconesSalvos() {
        ...
        $query = "SELECT * FROM GP_DESKTOP WHERE ID_USUARIO = '$idUsuarioLogado' ORDER BY ID";
    ...
    public function removerFavorito($nomeAba) {
        ...
        $query = "DELETE FROM GP_DESKTOP WHERE ID_USUARIO = '$idUsuarioLogado' AND NOME_ABA = '$nomeAba'";
        //print_r($query); exit();
    ...
    public function atualizaLocal($nomeAba, $x, $y) {
        ...
        $query = "UPDATE GP_DESKTOP SET X = '$x', Y = '$y' WHERE NOME_ABA = '$nomeAba' AND ID_USUARIO = '$idUsuarioLogado'";
        //print_r($query);
        return $this->conBanco->query($query);
    }
...
// insere um novo registro
    ...
    public function getListaUsuariosChat() {
        ...
        $query = "SELECT ID_EMPRESA FROM GP_CADASTRO_USUARIO WHERE ID = $idUsuarioLogado";
...
//**********************************************************************
        ...
        $query = "SELECT * FROM GP_CADASTRO_USUARIO T1
