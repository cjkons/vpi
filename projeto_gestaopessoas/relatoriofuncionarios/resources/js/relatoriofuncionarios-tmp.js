...
function carregarEmpresa(){
    ...
    $.ajax({
        url: 'index.php?m=relatoriofuncionarios&c=relatoriofuncionarioscontroller&f=carregarEmpresa',
        data: {
...
function carregarFilial(){
    ...
    $.ajax({
        url: 'index.php?m=relatoriofuncionarios&c=relatoriofuncionarioscontroller&f=carregarFilial',
        data: {
            idEmpresa: idEmpresa
...
function carregarDataAtual(){
     ...
     $.ajax({
        url: 'index.php?m=relatoriofuncionarios&c=relatoriofuncionarioscontroller&f=carregarDataAtual',
        data: {
...
function filtro(){
    ...
    if(controleDePreenchimento ==  'S'){
        ...
        $.ajax({
            url: 'index.php?m=relatoriofuncionarios&c=relatoriofuncionarioscontroller&f=filtro',
            data: {
                mes: mes,
                idEmpresa: idEmpresa,
                idFilial: idFilial
...
function getPdf() {
   ...
   var controleDePreenchimento = 'S';
    ...
    if(controleDePreenchimento ==  'S'){
        ...
        $.ajax({
            url: 'index.php?m=relatoriofuncionarios&c=relatoriofuncionarioscontroller&f=getPdf',
            data: {
...
function getExcel() {
   ...
   var controleDePreenchimento = 'S';
    ...
    $.ajax({
        url: 'index.php?m=relatoriofuncionarios&c=relatoriofuncionarioscontroller&f=getExcel',
        data: {
...
function abrirArquivoExcel(nomePastaTemporaria, nomeArquivo) {
    ...
    //LOCALHOST window.open('http://localhost/gestaopessoas/fwk//index.php?m=relatoriofuncionarios&c=relatoriofuncionarioscontroller&f=abrirArquivoExcel&nomePastaTemporaria=' + nomePastaTemporaria + '&nomeArquivo=' + nomeArquivo, '_blank'); 
    window.open('http://189.11.172.90/gestaopessoas/fwk/index.php?m=relatoriofuncionarios&c=relatoriofuncionarioscontroller&f=abrirArquivoExcel&nomePastaTemporaria=' + nomePastaTemporaria + '&nomeArquivo=' + nomeArquivo, '_blank');
    //alert(window);
}
function bloqueiaTela(texto) {
    ...
    $.blockUI({
        ...
        message: '<table class="carregando"><tr><td><img width="50px" src="resources/relatoriofuncionarios/img/001.gif" /></td><td style="font-size: 18px;">' + texto + '</td></tr></table>'
    });
}
