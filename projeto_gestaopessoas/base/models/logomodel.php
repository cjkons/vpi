...
class logomodel extends CI_Model {
    ...
    public function getIconesSalvos() {
        ...
        $query = "SELECT * FROM GP_DESKTOP WHERE ID_USUARIO = " . $this->getUsuarioLogado()->ID;
    ...
    public function atualizaLocal($nomeAba, $x, $y) {
        ...
        $query = "UPDATE GP_DESKTOP SET X = $x, Y = $y WHERE NOME_ABA = '$nomeAba' AND ID_USUARIO = " . $this->getUsuarioLogado()->ID;
    ...
    public function removerFavorito($nomeAba) {
        ...
        $query = "DELETE FROM GP_DESKTOP WHERE NOME_ABA = '$nomeAba' AND ID_USUARIO = " . $this->getUsuarioLogado()->ID;
        return $this->conBanco->query($query);
    }
